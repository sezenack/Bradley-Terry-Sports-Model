...
wb = openpyxl.load_workbook('Bradley-Terry Spreadsheet NBA.xlsx')
    ...
    for key in ratings:
        if key not in info:
            ...
            # Win ratio = wins / losses
                ...
                ratio = record[key][0] / record[key][1]
            # Add the team's win %, win ratio and sos is intialized to 0 (to be calculated later)
            teaminfo = [record[key][0] / (record[key][0] + record[key][1]), ratio, 0]
...
wb.save('Bradley-Terry Spreadsheet NBA.xlsx')
